...
// Public OAuth client IDs are not secrets. They are required in the authorize URL,
// but should be configured here instead of typed into the dashboard UI.
...
function getOAuthConfig(region) {
  if (window.location.hostname === 'localhost' && OAUTH_CLIENTS.localhost?.clientId && !OAUTH_CLIENTS.localhost.clientId.startsWith('REPLACE_')) {
    return OAUTH_CLIENTS.localhost;
  }
  const oauth = OAUTH_CLIENTS[region];
  if (!oauth || !oauth.clientId || oauth.clientId.startsWith('REPLACE_')) {
    throw new Error(`No Authorization Code + PKCE OAuth client ID is configured in app.js for region ${region}.`);
  }
  return oauth;
}
function getRedirectUri(region) {
  const oauth = getOAuthConfig(region);
  return oauth.redirectUri || (window.location.origin + window.location.pathname);
}
...
async function startLogin() {
  const cfg = getConfigFromUi();
  let oauth;
  try { oauth = getOAuthConfig(cfg.region); } catch (err) { alert(err.message); return; }
  localStorage.setItem(CONFIG_KEY, JSON.stringify(cfg));
  ...
  const redirectUri = getRedirectUri(cfg.region);
...
async function handleAuthCallback() {
  ...
  const redirectUri = getRedirectUri(saved.cfg.region);
  const body = new URLSearchParams();
  body.set('grant_type', 'authorization_code');
  body.set('client_id', getOAuthConfig(saved.cfg.region).clientId);
  body.set('code', code);
